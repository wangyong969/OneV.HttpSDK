...
namespace OneV.HttpSDK.Parser
{
    ...
    public class AopXmlParser<T> : IAopParser<T> where T : AopResponse
    {
        ...
        {
            string signNodeName = "<" + HttpSDKConstants.SIGN + ">";
            string signEndNodeName = "</" + HttpSDKConstants.SIGN + ">";
        ...
        {
            string rootNode = request.GetApiName().Replace(".", "_") + HttpSDKConstants.RESPONSE_SUFFIX;
            string errorRootNode = HttpSDKConstants.ERROR_RESPONSE;
        ...
        private static string ParseSignSourceData(string body, string rootNode, int indexOfRootNode)
        {
            ...
            int indexOfSign = body.IndexOf("<" + HttpSDKConstants.SIGN);
            if (indexOfSign < 0)
            {
                return null;
            }
        ...
        public string EncryptSourceData(IAopRequest<T> request, string body, string encryptType, string encryptKey, string charset)
        {
            ...
            string encryptContent = HttpSDKEncrypt.AesDencrypt(encryptKey, item.encryptContent, charset);
        ...
        private static EncryptParseItem ParseEncryptItem(string body, string rootNode, int indexOfRootNode)
        {
            ...
            string xmlStartNode = "<" + HttpSDKConstants.ENCRYPT_NODE_NAME + ">";
            string xmlEndNode = "</" + HttpSDKConstants.ENCRYPT_NODE_NAME + ">";
